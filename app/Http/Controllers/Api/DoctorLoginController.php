...
public function doctor_today_task()
{
    // --- AUTH ---
    $employee = auth('sanctum')->user();
    if (!$employee) {
    ...
    // --- FIND ROOM ASSIGNMENTS IDs for doctor ---
    $roomIds = RoomAssignment::where('employee_id', $employee->id)
        ->pluck('id');  // room_id = rooms.id (BIGINT)
    ...
    ->map(function($schedule) use ($today) {
        // agar schedule me time bhi hai, ya aapko specific "aaj" ka slice chahiye
        $schedule->for_date = $today; 
        return $schedule;
    });
    ...
    $consultations = PatientCheckup::with(['visit.user'])
    ->whereDate('checkup_date', $today) // aaj ke checkup_date
    ->whereHas('visit', function ($query) use ($employee) {
        $query->whereHas('consultantAssignment', function ($q) use ($employee) {
    ...
    $patientVisits = PatientVisit::whereDate('date_of_visit', $today)
        ->whereIn('department_consultant', $roomIds)
        ->with('user')
        ->get();

    // --- ROOM ASSIGNMENTS ---
    $roomAssignments = RoomAssignment::where('employee_id', $employee->id)
        ->with('room')
        ->get();

    // --- PROFESSIONS ---
    $employee->load('professions');
    ...
    return response()->json([
        ...
        'data' => [
            'schedules'        => $schedules,
            'appointments'     => $appointments,
            'today_checkup'    => $consultations,
            'today_visit'   => $patientVisits,
