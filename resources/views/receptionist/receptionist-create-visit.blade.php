...
<div class="min-h-screen">
  ...
  <div class="bg-white bg-white-800 rounded-lg shadow-lg p-6 border border-gray-200">
    <form id="createVisitForm" action="{{ route('visits.store', $user->id) }}" method="POST">
      @csrf
      <div class="space-y-6">
        <!-- Visit Details -->
        <div class="section">
              ...
              <select name="department_consultant"
               ...
               bg-white-700 transition duration-200">
                ...
                <option value="{{ $item->id }}">
                  Room: {{ $item->room->room_id }} — Doctor: {{ $item->employee->name }}
                </option>
                @endforeach
      ...
      fetch(this.action, {
        ...
        .then(data => {
          if (data.success) {
            ...
            setTimeout(() => {
              window.location.href = '{{ route("visits.store", $user->id) }}';
            }, 2000);
          } else {
