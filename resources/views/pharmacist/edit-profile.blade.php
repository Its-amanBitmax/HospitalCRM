...
<form method="POST" action="{{route('pharmacist.update.profile')}}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="section">
        ...
        <div class="grid-2">
            <input class="input" name="name" value="{{ $pharmacist->name }}" placeholder="Name">
            <input class="input" name="email" value="{{ $pharmacist->email }}" placeholder="Email">
            <input class="input" name="phone" value="{{ $pharmacist->phone }}" placeholder="Phone">
            ...
            <input type="date" class="input" name="date_of_birth" value="{{ $pharmacist->dob?\Carbon\Carbon::parse($pharmacist->dob)->format('Y-m-d'):'' }}">
            <input type="date" class="input" name="hire_date" value="{{ $pharmacist->hire_date?\Carbon\Carbon::parse($pharmacist->hire_date)->format('Y-m-d'):'' }}">
            <input class="input" name="status" value="{{ $pharmacist->status }}" placeholder="Status">
            <input class="input" name="department_id" value="{{ $pharmacist->department_id }}" placeholder="Department ID">
        </div>
    </div>
    ...
    <div class="section">
        ...
        @if($pharmacist->image)
        <img src="{{ asset('storage/'.$pharmacist->image) }}" class="w-24 h-24 mt-2 rounded">
        @endif
    </div>
    ...
    <div class="section">
        ...
        <div id="professions-container">
            @foreach($pharmacist->professions as $i=>$p)
    ...
    <div class="section">
        ...
        <div id="addresses-container">
            @foreach($pharmacist->addresses as $i=>$a)
    ...
    <div class="section">
        ...
        <div id="qualifications-container">
            @foreach($pharmacist->qualifications as $i=>$q)
            <div data-box class="grid-3 border p-3 mb-3 rounded">
    ...
    <div class="section">
        ...
        <div id="family-container">
            @foreach($pharmacist->familyDetails as $i=>$f)
            <div data-box class="grid-2 border p-3 mb-3 rounded">
    ...
    <div class="section">
        ...
        @foreach($pharmacist->documents as $doc)
...
<script>
    let addressIndex = {{ $pharmacist->addresses->count() }};
    let professionIndex = {{ $pharmacist->professions->count() }};
    let qualificationIndex = {{ $pharmacist->qualifications->count() }};
    let familyIndex = {{ $pharmacist->familyDetails->count() }};
    ...
    function removeItem(btn, type, id) {
        ...
        if (id) {
            ...
            document.getElementById('deleted_' + type).value = JSON.stringify(deleted[type]);
        }
    }
