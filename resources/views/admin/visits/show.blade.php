...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <h1 class="text-2xl font-bold text-gray-800">Hospital Visit Details</h1>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <p class="text-gray-900">{{ $visit->visitor_name }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Contact</label>
                            <p class="text-gray-900">{{ $visit->visitor_contact ?: 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Email</label>
                            <p class="text-gray-900">{{ $visit->visitor_email ?: 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Relation</label>
                            <p class="text-gray-900">{{ $visit->visitor_relation ?: 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Contact Person Name</label>
                            <p class="text-gray-900">{{ $visit->contact_person_name ?: 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Contact Person Phone</label>
                            <p class="text-gray-900">{{ $visit->contact_person_phone ?: 'N/A' }}</p>
                        </div>
                        @if($visit->invitation_code)
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Invitation Code</label>
                            <p class="text-gray-900 font-mono">{{ $visit->invitation_code }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="space-y-3">
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Visit Type</label>
                        ...
                        <div>
                            ...
                            <p class="text-gray-900">{{ $visit->purpose ?: 'N/A' }}</p>
                        </div>
                        @if($visit->invite_status != 'none')
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Invite Status</label>
                            <span class="px-2 py-1 text-xs font-semibold rounded-full
                ...
                <div class="bg-white-50 bg-white-700 p-4 rounded-lg">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <p class="text-gray-900 ">{{ $visit->scheduled_visit ? $visit->scheduled_visit->format('M d, Y H:i') : 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600 text-gray-400">Check In</label>
                            <p class="text-gray-900 ">{{ $visit->check_in ? $visit->check_in->format('M d, Y H:i') : 'Not checked in' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600 text-gray-400">Check Out</label>
                            <p class="text-gray-900 ">{{ $visit->check_out ? $visit->check_out->format('M d, Y H:i') : 'Not checked out' }}</p>
                        </div>
                        @if($visit->invited_at)
                        <div>
                            <label class="block text-sm font-medium text-gray-600 text-gray-400">Invited At</label>
                            <p class="text-gray-900 ">{{ $visit->invited_at->format('M d, Y H:i') }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            @if($visit->patient)
                                <p class="text-gray-900">{{ $visit->patient->name }}</p>
                                <p class="text-sm text-gray-600">{{ $visit->patient_mr_no ?: 'No MR Number' }}</p>
                            @else
                ...
                <div class="bg-white-50 bg-white-700 p-4 rounded-lg">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <p class="text-gray-900 ">{{ $visit->id_proof_type ? ucwords(str_replace('_', ' ', $visit->id_proof_type)) : 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600 text-gray-400">ID Proof Number</label>
                            <p class="text-gray-900 ">{{ $visit->id_proof_number ?: 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600 text-gray-400">Badge Number</label>
                            <p class="text-gray-900 ">{{ $visit->badge_number ?: 'N/A' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg lg:col-span-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-gray-900">{{ $visit->creator ? $visit->creator->name : 'System' }}</p>
                            <p class="text-sm text-gray-600">{{ $visit->created_at->format('M d, Y H:i') }}</p>
                        </div>
                        @if($visit->updater)
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Last Updated By</label>
                            <p class="text-gray-900">{{ $visit->updater->name }}</p>
                            <p class="text-sm text-gray-600">{{ $visit->updated_at->format('M d, Y H:i') }}</p>
                        </div>
                        @endif
                    </div>
                </div>
