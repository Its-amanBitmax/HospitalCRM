...
<div class="min-h-screen">
  ...
  <div class="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-6">
    ...
    <div class="flex gap-3">
      <a href="{{ route('admin.users.show', $user->id) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-900 px-4 py-2 rounded-lg transition duration-200 shadow-md hover:shadow-lg">
        ...
        <table class="min-w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Associated Visit</th>
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $checkup->checkup_date?->format('d-m-Y') ?? '-' }}</td>
            ...
            @empty
            <tr>
              <td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">No checkups found</td>
            </tr>
            @endforelse
          </tbody>
        </table>
    ...
    <div id="summary-content" class="tab-content">
      <div class="space-y-6">
        ...
        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">Patient Summary</h2>
          <div class="flex flex-col md:flex-row gap-6 mb-6">
            <!-- Patient Image -->
            <div class="flex-shrink-0">
              @if($user->image)
                <img src="{{ asset($user->image) }}" alt="Patient Image" class="w-24 h-24 rounded-full object-cover border-4 border-white dark:border-gray-600 shadow-lg">
              @else
                <div class="w-24 h-24 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center">
                  <i class="fas fa-user text-3xl text-gray-600 dark:text-gray-400"></i>
                </div>
              @endif
            </div>
            <!-- Patient Details -->
            <div class="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Full Name:</span> <span class="text-gray-900 dark:text-white">{{ $user->full_name }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Email:</span> <span class="text-gray-900 dark:text-white">{{ $user->email ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Phone:</span> <span class="text-gray-900 dark:text-white">{{ $user->mobile_no ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Alternate No:</span> <span class="text-gray-900 dark:text-white">{{ $user->alternate_no ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Date of Birth:</span> <span class="text-gray-900 dark:text-white">{{ $user->date_of_birth?->format('d-m-Y') ?? '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Age:</span> <span class="text-gray-900 dark:text-white">{{ $user->age ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Gender:</span> <span class="text-gray-900 dark:text-white">{{ $user->gender ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Blood Group:</span> <span class="text-gray-900 dark:text-white">{{ $user->blood_group ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Father/Spouse:</span> <span class="text-gray-900 dark:text-white">{{ $user->father_spouse_name ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Emergency Contact:</span> <span class="text-gray-900 dark:text-white">{{ $user->emergency_contact ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">ID Proof:</span> <span class="text-gray-900 dark:text-white">{{ $user->id_proof_type ?: '-' }} {{ $user->id_number ? '('.$user->id_number.')' : '' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Registration Date:</span> <span class="text-gray-900 dark:text-white">{{ $user->created_at?->format('d-m-Y') ?? '-' }}</span></div>
            </div>
          </div>
          <!-- Address Section -->
          <div class="border-t border-gray-200 dark:border-gray-600 pt-4">
            <h3 class="text-lg font-medium text-gray-800 dark:text-white mb-2">Address Information</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Full Address:</span> <span class="text-gray-900 dark:text-white">{{ $user->full_address ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">City:</span> <span class="text-gray-900 dark:text-white">{{ $user->city ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">State:</span> <span class="text-gray-900 dark:text-white">{{ $user->state ?: '-' }}</span></div>
              <div><span class="font-medium text-gray-700 dark:text-gray-300">Pin Code:</span> <span class="text-gray-900 dark:text-white">{{ $user->pin_code ?: '-' }}</span></div>
            </div>
          </div>
        </div>
        ...
        <!-- Recent Visits -->
        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">Recent Visits (Last 2)</h2>
          @if($visits->count() > 0)
            @foreach($visits->take(2) as $visit)
              <div class="bg-white dark:bg-gray-800 p-4 rounded-lg mb-3 border border-gray-200 dark:border-gray-600">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  <div><span class="font-medium text-gray-700 dark:text-gray-300">Type:</span> {{ $visit->visit_type }}</div>
                  <div><span class="font-medium text-gray-700 dark:text-gray-300">Date:</span> {{ $visit->date_of_visit?->format('d-m-Y') ?? '-' }}</div>
                  <div class="md:col-span-2"><span class="font-medium text-gray-700 dark:text-gray-300">Complaint:</span> {{ $visit->chief_complaint ?: '-' }}</div>
                  <div><span class="font-medium text-gray-700 dark:text-gray-300">Referred By:</span> {{ $visit->referred_by ?: '-' }}</div>
                  <div><span class="font-medium text-gray-700 dark:text-gray-300">Consultant:</span> {{ $visit->department_consultant ?: '-' }}</div>
                </div>
              </div>
            @endforeach
          @else
            <p class="text-gray-500 dark:text-gray-400">No visits recorded.</p>
          @endif
        </div>

        <!-- All Checkups -->
        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">All Checkups</h2>
          @if($checkups->count() > 0)
            <div class="space-y-3">
              @foreach($checkups as $checkup)
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-600">
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    <div><span class="font-medium text-gray-700 dark:text-gray-300">Date:</span> {{ $checkup->checkup_date?->format('d-m-Y') ?? '-' }}</div>
                    <div><span class="font-medium text-gray-700 dark:text-gray-300">Visit:</span>
                      @if($checkup->visit)
                        {{ $checkup->visit->date_of_visit?->format('d-m-Y') }} - {{ $checkup->visit->visit_type }}
                      @else
                        -
                    ...
                    <div class="md:col-span-2"><span class="font-medium text-gray-700 dark:text-gray-300">Diagnosis:</span> {{ $checkup->diagnosis ?: '-' }}</div>
                    <div class="md:col-span-2"><span class="font-medium text-gray-700 dark:text-gray-300">Treatment:</span> {{ $checkup->treatment ?: '-' }}</div>
                  </div>
                </div>
              @endforeach
            </div>
          @else
            <p class="text-gray-500 dark:text-gray-400">No checkups recorded.</p>
          @endif
        </div>

        <!-- All Documents -->
        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">All Documents</h2>
          @if($documents->count() > 0)
            <div class="space-y-3">
              @foreach($documents as $document)
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-600 flex justify-between items-center">
                  <div class="text-sm">
                    <p class="font-medium text-gray-900 dark:text-white">{{ $document->document_type }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Uploaded: {{ $document->created_at?->format('d-m-Y H:i') }}</p>
                  </div>
