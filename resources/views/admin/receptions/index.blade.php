...
<div class="min-h-screen">
    ...
    <div class="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-6">
        <div class="flex items-center gap-3">
            <i class="fas fa-concierge-bell text-2xl text-blue-600 dark:text-blue-400"></i>
            <h1 class="text-xl font-semibold text-gray-800 dark:text-white">Reception Management</h1>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow flex items-center gap-3">
            ...
            <div>
                <div class="text-2xl font-bold text-gray-800 dark:text-white">{{ $totalReceptions }}</div>
                <div class="text-sm text-gray-600 dark:text-gray-400">Total Receptions</div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow flex items-center gap-3">
            <i class="fas fa-check-circle text-3xl text-green-600 dark:text-green-400"></i>
            <div>
                <div class="text-2xl font-bold text-gray-800 dark:text-white">{{ $activeReceptions }}</div>
        ...
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow flex items-center gap-3">
            ...
            <div>
                <div class="text-2xl font-bold text-gray-800 dark:text-white">{{ $inactiveReceptions }}</div>
    ...
    <div class="mb-6 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Search Reception</label>
                <div class="flex gap-2">
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Search by reception ID or employee"
                        class="flex-1 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-800">
    ...
    <div class="overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-900">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Reception ID</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Assigned Employee</th>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $rec->reception_id }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $rec->employee->name ?? 'Not Assigned' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $rec->status == 'active' ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-200' }}">
                            {{ ucfirst($rec->status) }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="flex justify-center items-center gap-2">
                            ...
                            <button type="button"
                                ...
                                onclick="openViewModal('{{ $rec->reception_id }}','{{ $rec->employee->name ?? ''}}','{{ $rec->status }}')">
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">No Receptions Found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="addReceptionModal" class="fixed inset-0 hidden z-50 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 class="text-xl font-bold mb-4 text-gray-800 dark:text-gray-100">Add Reception</h2>
        <form action="{{ route('admin.reception.store') }}" method="POST">
            @csrf
            <label class="block mb-2 text-gray-700 dark:text-gray-300">Reception ID</label>
            <input type="text" name="reception_id" value="{{ $nextReceptionId }}" class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 mb-4" readonly>

            <label class="block mb-2 text-gray-700 dark:text-gray-300">Status</label>
            <select name="status" class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 mb-4">
...
<div id="viewModal" class="fixed inset-0 hidden bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 class="text-xl font-bold mb-4 text-gray-800 dark:text-gray-100">Reception Details</h2>
        <div id="viewModalData" class="text-gray-700 dark:text-gray-300 space-y-2"></div>
    ...
    <div class="bg-white rounded-xl w-full max-w-md p-6 relative shadow-lg">
        ...
        <form id="assignForm" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="employee_id" id="employee_id"
                    ...
                    <option value="{{ $emp->employee->id }}"
                        {{ isset($reception) && $reception->assigned_employee == $emp->employee->id ? 'selected' : '' }}>
            ...
            <div class="flex justify-end gap-3 mt-6">
                <button type="button" id="cancelAssignBtn" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 transition">Cancel</button>
...
<div id="editModal" class="fixed inset-0 hidden bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 class="text-xl font-bold mb-4 text-gray-800 dark:text-gray-100">Edit Reception</h2>
        <form action="{{ route('admin.reception.update') }}" method="POST">
            @csrf
            <input type="hidden" name="id" id="editId">

            <label class="block mb-2 text-gray-700 dark:text-gray-300">Reception ID</label>
            <input type="text" name="reception_id" id="editReceptionId" class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 mb-4">

            <label class="block mb-2 text-gray-700 dark:text-gray-300">Assigned Employee</label>
            <select name="employee_id" id="editAssignedEmployee"
                class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 mb-4">
            ...
            <label class="block mb-2 text-gray-700 dark:text-gray-300">Status</label>
            <select name="status" id="editStatus" class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 mb-4">
...
<script>
    ...
    // View Modal
    function openViewModal(id, assigned, status) {
        document.getElementById('viewModalData').innerHTML = `<p><b>Reception ID:</b> ${id}</p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.assignBtn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const assignedEmployeeId = this.dataset.assignedEmployeeId; // 🔥 new

                modal.style.display = 'flex';
                modalTitle.textContent = `Assign Employee to ${receptionName}`;

                assignForm.action = `/receptions/${receptionId}/assign`;

                // Set the currently assigned employee as selected
                if (assignedEmployeeId) {
                    employeeSelect.value = assignedEmployeeId;
                } else {
                    employeeSelect.value = ""; // no selection if nothing assigned
                }
            });
        });
